...
def test_yaml_registry_builds_deterministic_strategies():
    # 5 frozen controls + 25 v1 hypothesis fleet + 25 v2 retune fleet
    # (2026-07-17) + 4 v3 cost variants (2026-07-21), minus liquidity_fade_v2
    # (retired 2026-07-19, cash below min stake).
    from polybot.config import load_config
    cfg = load_config("config.yaml")
    strats = [s for s in build_strategies(cfg) if s.kind != "ai_shadow"]
    assert len(strats) == 58
    assert len({s.name for s in strats}) == 58
